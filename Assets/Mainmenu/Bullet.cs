...
public class Bullet: MonoBehaviour {
	...
	void OnCollisionEnter2D(Collision2D col) {
		...
		if (col.gameObject.CompareTag("Enemy") && !IsAggro) {
			...
			if (Optionsloaded.isSFXOn) gm.GetComponent < AudioSource > ().Play();
		...
		if (col.gameObject.CompareTag("Player") && IsAggro) {
			gm.GameState = GM.state.playerdied;
	...
	void Update() {
		this.gameObject.transform.position += new Vector3(0, speed * ((Time.timeScale != 0) ? Time.unscaledDeltaTime: 0), 0);
	}
}