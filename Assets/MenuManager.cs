...
public static class Optionsloaded
{
	...
	public static bool isMusicOn { get { return musicON; } set { musicON = value; } }
	private static bool sfxON = true;
	public static bool isSFXOn { get { return sfxON; } set { sfxON = value; } }
}
	...
	void Update () {
		...
		if (Input.GetButtonDown ("Fire1") == true) {
			...
			if (ev == "setfps") {
				...
				buttons [ButtonSelected].UID.GetComponent<Text> ().text = 
					"Framerate: " + Application.targetFrameRate.ToString ();
			}
			if (ev == "setmusic") {
				Optionsloaded.isMusicOn = !Optionsloaded.isMusicOn;
				buttons [ButtonSelected].UID.GetComponent<Text> ().text = 
					"Music: " + (Optionsloaded.isMusicOn ? "On" : "Off");
			}
			if (ev == "setsfx") {
				Optionsloaded.isSFXOn = !Optionsloaded.isSFXOn;
				buttons [ButtonSelected].UID.GetComponent<Text> ().text = 
					"SFX: " + (Optionsloaded.isSFXOn ? "On" : "Off");
			}
			if (ev == "quitgame") {
				Application.Quit ();
			} 	
		...
		selector.GetComponent<RectTransform> ().anchoredPosition = 
			new Vector2(3, buttons[ButtonSelected].UID.GetComponent<RectTransform>().anchoredPosition.y);
}
